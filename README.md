...
公平锁是指多个线程按照申请锁的顺序来获取锁，线程直接进入队列中排队，队列中的第一个线程才能获取锁。

- 公平锁的优点：等待锁的线程不会饿死，
- 公平锁的缺点：整体吞吐效率相对于非公平锁要低。
- 等待队列中除了第一个线程以外所有线程都会阻塞，CPU 唤醒阻塞线程的开销比非公平锁大。

非公平锁是多个线程加锁时直接尝试获取锁，获取不到才会进入等待队列的队尾等待。但如果此时锁刚好可以使用，那么这个线程可以无需阻塞直接获取到锁，所以非公平锁有可能出现后申请锁的线程反而先获取到锁，

- 非公平锁的优点：可以减少唤起线程的开销，整体的吞吐效率高。因为线程有几率不阻塞，直接获取锁，CUP 不必唤醒所有线程。
- 非公平锁的缺点：处于等待队列中的线程可能会饿死，或者等很久才会获得锁。 
...
>
> *分析读写锁 ReentrantReadWriteLock*，会发现它有个潜在的问题：读锁全完，写锁有望；写锁独占，读写全堵；
>
...
邮戳锁 StampedLock，是 JDK1.8 中新增的一个读写锁，是对 JDK1.5 中的读写锁 ReentrantReadWriteLock 的优化。stamp（戳记，long类型），代表了锁的状态。当 stamp 返回零时，表示线程获取锁失败。并且，当释放锁或者转换锁的时候，都要传入最初获取的 stamp 值。
